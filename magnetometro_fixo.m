...
%o angulo entre o norte real e o norte magnético. Mantendo a leitura do
%magnetômetro constante, para cada instante de tempo é adicionado um ruído
%à medição, podemos ver a inflência do ruído no angulo de yaw do drone.

% Parâmetros do magnetômetro
num_amostras = 100; % Número de amostras/tempo
MAG = randn(3, 1); % Leitura constante do magnetômetro (Earth's magnetic field)
MAG = repmat(MAG, 1, num_amostras); % Repetir a leitura constante ao longo do tempo
...
subplot(2, 2, 1);
plot(1:num_amostras, MAG(1:3, 1) * ones(1, num_amostras));
